...
namespace Howlite {
	...
	template<typename ObjectType, size_t MemoryBlockCapacity>
	class HL_API TDynamicObjectPool {
		// we would like to set object size to be aligned to system word size,
		// if it's not already aligned (to avoid situation with packed structures
		// and keep possibility to construct internal free list)
		static constexpr size_t OBJECT_SIZE = sizeof(ObjectType) % sizeof(void*) ?
		...
		// we would like to set memory block size to be capable to fit required object count plus add
		// additional space for possible align of given memory pointer
		static constexpr size_t MEMORY_BLOCK_SIZE = OBJECT_SIZE * MemoryBlockCapacity + sizeof(void*);
		static constexpr size_t MEMORY_BLOCK_CAPACITY = MemoryBlockCapacity;

		struct MemoryBlock {
			HL_CLASS_DISABLE_COPY(MemoryBlock);


		};

	public:
		HL_DISABLE_COPY(TDynamicObjectPool);

		TDynamicObjectPool(const std::string& InMemoryUser)
		{
		...
		void DestroyObject(ObjectType* InObject)
		{
